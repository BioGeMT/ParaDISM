...
# Iterative FreeBayes variant calling with consensus reference improvement
OUTPUT_DIR="${1:-./output}"
REF_FILE="${2:-ref.fa}"
ITERATIONS="${3:-2}"

echo "=== Iterative FreeBayes Pipeline ==="
...
for ((i=1; i<=ITERATIONS; i++)); do
    ...
    echo "Running FreeBayes variant calling against original ref..."
    ../../variant_calling_freebayes.sh "." "../../$CALL_REF"
...
EOF
        ...
        for j in "${!GENES[@]}"; do
            ...
            if [[ -f "variant_calling/${mapper_name}_freebayes_norm.vcf" ]]; then
                # Keep SNPs only and compress for bcftools consensus
                bcftools view -v snps "variant_calling/${mapper_name}_freebayes_norm.vcf" | \
        ...
        # Combine consensus sequences only if any were produced
        shopt -s nullglob
        ...
        if (( ${#CONS_FILES[@]} > 0 )); then
            ...
            if [[ -s "variant_calling/consensus_reference.fa" ]]; then
                ...
                echo "Warning: consensus_reference.fa is empty; keeping previous mapping reference ($MAP_REF)"
            fi
        else
            echo "No per-gene consensus sequences found; keeping previous mapping reference ($MAP_REF)"
        fi
        shopt -u nullglob
    ...
    # Clean up mapper intermediate files (keep only essential outputs)
    echo "Cleaning up mapper intermediate files..."
    rm -f PKD1_index.*.bt2
...
echo ""
echo "=== Iterative FreeBayes Pipeline Complete ==="
