...
def process_files(tsv_file, fastq_file, sam_file, output_folder):
    import os
    ...
    # Save mapping TSVs
    tsv_df.to_csv(os.path.join(output_folder, 'tsv_mapping.tsv'), sep='\t', index=False)
    sam_df.to_csv(os.path.join(output_folder, 'sam_mapping.tsv'), sep='\t', index=False)
    ...
    for df, suffix in [(tsv_df, 'tsv'), (sam_df, 'sam')]:
        ...
        cm.to_csv(os.path.join(output_folder, f'{suffix}_confusion_matrix.tsv'), sep='\t')
        ...
        plt.savefig(os.path.join(output_folder, f'{suffix}_confusion_matrix.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'accuracy_comparison.png'), dpi=300)
    plt.close()

def main():
    # Set up argument parser
    parser = argparse.ArgumentParser(description='Process gene mapping files and generate comparison results.')
    parser.add_argument('--tsv', required=True, help='Path to the TSV file with unique mappings')
    parser.add_argument('--fastq', required=True, help='Path to the FASTQ file')
    parser.add_argument('--sam', required=True, help='Path to the SAM file')
    ...
    # Use the arguments
    process_files(args.tsv, args.fastq, args.sam, args.output)
    
    # Use os.path.join to construct paths in the output message
    import os
    output_folder = args.output
    print(f"Files generated in {output_folder}:\n"
          f"- {os.path.join(output_folder, 'tsv_mapping.tsv')}\n"
          f"- {os.path.join(output_folder, 'sam_mapping.tsv')}\n"
          f"- {os.path.join(output_folder, 'tsv_confusion_matrix.tsv')}\n"
          f"- {os.path.join(output_folder, 'sam_confusion_matrix.tsv')}\n"
          f"- {os.path.join(output_folder, 'tsv_confusion_matrix.png')}\n"
          f"- {os.path.join(output_folder, 'sam_confusion_matrix.png')}\n"
          f"- {os.path.join(output_folder, 'accuracy_comparison.png')}")
