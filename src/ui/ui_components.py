...
def display_pipeline_config(
    ...
    """
    Display full pipeline configuration before execution.

    Args:
        r1_file: R1 FASTQ filename
        r2_file: R2 FASTQ filename
        ref_file: Reference FASTA filename
        read_count: Number of read pairs
        ...
        minimap2_profile: Optional minimap2 profile (short/pacbio/nanopore)
    """
    lines = []
    ...
    if sam_file:
        ...
        aligner_display = f"{aligner.upper()}"
        if minimap2_profile:
            aligner_display += f" ({minimap2_profile})"
