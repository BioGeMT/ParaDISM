...
python analysis/read_mapping_analysis.py -t output/unique_mappings.tsv -f simulated_r1.fq -s output/mapped_reads.sam -o analysis/read_mapping_analysis -p read_mapping_analysis

python analysis/snp_calling_analysis.py all_genes_mutations.vcf analysis/mapper1.vcf analysis/mapperp1.vcf analysis/mapperp2.vcf analysis/mapperp3.vcf analysis/mapperp4.vcf analysis/mapperp5.vcf analysis/mapperp6.vcf --method2 analysis/bowtie2.vcf --tsv output/unique_mappings.tsv --sam output/mapped_reads.sam --fastq simulated_r1.fq --output-dir analysis/variant_calling_analysis
