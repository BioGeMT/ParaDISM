...
#
# Usage:
#   bash benchmark/giab/filter_simple_snps_acgt_final.sh \
#     --input-vcf <in.vcf.gz> \
...
if [[ -z "$BCFTOOLS" && -x "$HOME/miniconda3/envs/paradism/bin/bcftools" ]]; then
  BCFTOOLS="$HOME/miniconda3/envs/paradism/bin/bcftools"
fi
if [[ -z "$BCFTOOLS" ]]; then
  echo "Error: bcftools not found in PATH and not found in paradism." >&2
  exit 1
fi
