...
# Run from anywhere: bash benchmark/giab/prepare_giab_truth.sh
...
download_if_missing() {
    ...
    if command -v wget >/dev/null 2>&1; then
        ...
        echo "ERROR: neither wget nor curl is available for downloading GIAB files." >&2
        exit 1
    fi
}
...
if [[ -z "$BCFTOOLS" || -z "$BGZIP" ]]; then
    if command -v conda >/dev/null 2>&1; then
        BCFTOOLS="$(conda run -n paradism which bcftools 2>/dev/null || true)"
        BGZIP="$(conda run -n paradism which bgzip 2>/dev/null || true)"
    fi
fi
if [[ -z "$BCFTOOLS" || -z "$BGZIP" ]]; then
    echo "ERROR: bcftools/bgzip not found on PATH (and conda env 'paradism' not available)." >&2
    exit 1
fi
...
BENCHMARK_VCF_TBI="${BENCHMARK_VCF}.tbi"
BENCHMARK_BED="$OUTPUT_DIR/HG002_GRCh38_1_22_v4.2.1_benchmark_noinconsistent.bed"
GIAB_BASE_URL="https://ftp-trace.ncbi.nlm.nih.gov/giab/ftp/release/AshkenazimTrio/HG002_NA24385_son/NISTv4.2.1/GRCh38"

download_if_missing "${GIAB_BASE_URL}/HG002_GRCh38_1_22_v4.2.1_benchmark.vcf.gz" "$BENCHMARK_VCF"
download_if_missing "${GIAB_BASE_URL}/HG002_GRCh38_1_22_v4.2.1_benchmark.vcf.gz.tbi" "$BENCHMARK_VCF_TBI"
download_if_missing "${GIAB_BASE_URL}/HG002_GRCh38_1_22_v4.2.1_benchmark_noinconsistent.bed" "$BENCHMARK_BED"

# Regions match ParaDISM reference contigs (benchmark/references/pkd1_panel.fa) and
...
python3 "$SCRIPT_DIR/make_benchmark_gene_coords_bed.py" --benchmark-bed "$BENCHMARK_BED"
