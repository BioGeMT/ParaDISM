...
1-based contig coordinates matching benchmark/references/pkd1_panel.fa.
"""
...
def _bgzip_and_index(vcf_path: Path, out_vcf_gz: Path) -> None:
    ...
    Prefers system tools; falls back to `conda run -n paradism`.
    """
    ...
    if not (bgzip and bcftools):
        ...
        bgzip_p = subprocess.run(
            [conda, "run", "-n", "paradism", "which", "bgzip"],
            capture_output=True,
            text=True,
            check=True,
        ).stdout.strip()
        bcftools_p = subprocess.run(
            [conda, "run", "-n", "paradism", "which", "bcftools"],
            capture_output=True,
            text=True,
            check=True,
        ).stdout.strip()
...
def main() -> int:
    ...
    parser.add_argument(
        "--ref-fasta",
        type=Path,
        default=REPO_ROOT / "benchmark/references/pkd1_panel.fa",
        help="Reference FASTA used by ParaDISM per-gene contigs (default: benchmark/references/pkd1_panel.fa).",
    )
    parser.add_argument(
        "--output",
        type=Path,
