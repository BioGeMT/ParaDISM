...
#
# Strategy:
# - Uses pre-split shard files under giab_hg002_reads (D1_*_R1_*.fastq.gz / R2).
...
#
# Usage:
#   ./benchmark/giab/downsample_hg002_fastq.sh [options]
#
# Options:
...
read1_shards=()
while IFS= read -r read1_shard; do
  read1_shards+=("$read1_shard")
done < <(
  find "$READS_DIR" -maxdepth 1 -type f \( -name 'D1_*_R1_*.fastq' -o -name 'D1_*_R1_*.fastq.gz' \) |
    LC_ALL=C sort
)
if (( ${#read1_shards[@]} == 0 )); then
  echo "Error: no shard FASTQs found in $READS_DIR (expected D1_*_R1_*.fastq or .fastq.gz)." >&2
  exit 1
fi
...
stream_command() {
  local path="$1"
  local quoted_path
  quoted_path="$(quote_for_shell "$path")"
  if [[ "$path" == *.gz ]]; then
    printf "gzip -cd -- %s" "$quoted_path"
  else
    printf "cat -- %s" "$quoted_path"
  fi
}
...
if (( leftover > 0 )); then
  ranked_indices=()
  while IFS= read -r ranked_index; do
    ranked_indices+=("$ranked_index")
  done < <(
    for i in "${!read1_shards[@]}"; do
...
for i in "${!read1_shards[@]}"; do
  ...
  read1_cmd="$(stream_command "$read1_shard")"
  read2_cmd="$(stream_command "$read2_shard")"
  ...
  copied_pairs="$(
    "$awk_bin" \
      ...
      -v read1_cmd="$read1_cmd" \
      -v read2_cmd="$read2_cmd" \
      -v max_pairs="$requested_pairs" \
      -v progress_every="$PROGRESS_EVERY" \
      -v already_written="$written_pairs_total" '
BEGIN{
  copied=0;
  while (copied < max_pairs && (read1_cmd | getline header1) > 0) {
    if ((read1_cmd | getline sequence1) <= 0 || (read1_cmd | getline plus1) <= 0 || (read1_cmd | getline quality1) <= 0) exit 2;
    if ((read2_cmd | getline header2) <= 0 || (read2_cmd | getline sequence2) <= 0 || (read2_cmd | getline plus2) <= 0 || (read2_cmd | getline quality2) <= 0) exit 3;
