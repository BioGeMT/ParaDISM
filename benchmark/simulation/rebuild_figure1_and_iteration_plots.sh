...
#
# Usage examples:
#   bash benchmark/simulation/rebuild_figure1_and_iteration_plots.sh
#   bash benchmark/simulation/rebuild_figure1_and_iteration_plots.sh --seed-start 1 --seed-end 1000
#   bash benchmark/simulation/rebuild_figure1_and_iteration_plots.sh --sim-output-base results/simulation/pkd1_panel
#
...
SIM_OUTPUT_BASE="results/simulation/pkd1_panel"
SEED_START=1
SEED_END=1000
ALIGNERS="bwa-mem2 bowtie2 minimap2"
CONDA_ENV="${CONDA_ENV:-paradism}"
FIGURE1_DATA_DIR=""
ITERATION_PLOTS_DIR=""
OUTPUT_SVG=""
...
for aligner in "${ALIGNER_LIST[@]}"; do
    ...
    mkdir -p "${out_dir}"
    run_py benchmark/simulation/build_figure1_csv.py \
...
for aligner in "${ALIGNER_LIST[@]}"; do
    run_py benchmark/simulation/plot_iteration_progression.py \
        --sim-output-base "${SIM_OUTPUT_BASE}" \
        --seed-start "${SEED_START}" \
        --seed-end "${SEED_END}" \
...
run_py benchmark/simulation/make_paper_multipanel_plot.py \
