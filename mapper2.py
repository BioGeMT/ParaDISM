...
SRC_DIR = Path(__file__).parent / "src2"
...
def run_with_arguments(args: argparse.Namespace) -> None:
    ...
    if not Path(args.reference).exists():
        ...
        sys.exit(1)
    if args.sam:
        console.print("[red]✗ --sam is not supported in mapper2 (per-gene alignment mode)[/red]")
        sys.exit(1)
...
Examples:
  ...
  python mapper.py --read1 ont_q20.fq --reference ref.fa --aligner minimap2 --minimap2-profile ont-q20
    ...
    optional.add_argument(
        "--sam",
        metavar="SAM",
        help="[mapper2] Not supported; provided for CLI parity and will raise an error if used",
    )
    optional.add_argument(
        "--aligner",
        metavar="ALIGNER",
